...
Especial used for the dataframe that has two columns and limited rows
Suffix.df, Collective.df, Season.df
```{r}
...
Function: dataExists
Checks if it already exists in the passed data frame. Returns a key to the item if it exists, 0 otherwise and add it to the hashmap which makes checking data much faster
Especial used for the dataframe that has more than two columns and multiple rows
  ...
  if (nAuthor > 0) {
      for (i in 1 : nAuthor) {
        # check if each author already exists
         ...
         pk.author <- dataExists(authorsHashmap, Authors.df, name)
        if (pk.author == 0) {
